...
export type MemoryInspectorSection = "memory" | "reflection";

type Props = {
  worldId: string;
  characterId: string;
  section: MemoryInspectorSection;
};

export function MemoryInspectorPanel({ worldId, characterId, section }: Props) {
  ...
  if (loading) {
    return <p className="memory-muted">Loading…</p>;
  }
        ...
        <>
          <FormSection title="Reflection">
            ...
            <p className="memory-muted">
              Consolidates recent diary into beliefs, lessons, and memory links. Nightly runs when
              reflection is enabled in world policy.
            </p>
          </FormSection>
